...
class GcEventMonitoringSessionController(project: Project) : ProtocolSubscribedProjectComponent(project) {
    ...
    init {
        hostModel.gcEventsMonitoringSessions.view(projectComponentLifetime) { lt, _, session ->
    ...
    fun startSession(pid: Int, model: MonitorGcEventsModel) {
        ...
        val command = MonitorGcEventsCommand(pid, duration)

        val monitorTask = hostModel.monitorGcEvents.start(sessionDefinition.lifetime, command)
        sessionStarted(pid)
    ...
    fun startExistingSession(pid: Int, model: MonitoringTimerModel) {
        ...
        val session = hostModel.gcEventsMonitoringSessions[pid] ?: return
        val duration = if (model.stoppingType == StoppingType.AfterPeriod) model.duration else null
    ...
    private fun viewSession(lt: Lifetime, session: GcEventsMonitoringSession) {
