...
internal sealed class TraceMonitoringHandler
{
    public TraceMonitoringHandler(ISolution solution, Lifetime lifetime)
    {
        ...
        hostModel.TraceMonitoringSessions.View(lifetime, (lt, pid, session) => Handle(lt, pid, session));
    }

    private static void Handle(Lifetime lt, int pid, TraceMonitoringSession session)
    {
