...
class GcEventSessionController(project: Project) : SessionController<GcEventSession, GcEventSessionModel>(project) {
    companion object {
        fun getInstance(project: Project): GcEventSessionController = project.service()
        private const val GC_EVENTS = "GC events"
    }

    override val artifactType = GC_EVENTS

    override fun getSessions() = DiagnosticsHost.getInstance(project).hostModel?.gcEventSessions

    override fun addSessionTab(pid: Int, session: GcEventSession, sessionLifetime: Lifetime) {
        GcEventSessionTabManager.getInstance(project).addSessionTab(sessionLifetime, pid, session)
    }

    override fun createSession(model: GcEventSessionModel) = GcEventSession()
}